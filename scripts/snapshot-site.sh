#!/usr/bin/env bash
#
# QFLASH21 v2.1.0 STANDALONE – spiegelt die komplette Produktions-Website
...
#   4. Versions-Patch: Snapshot-Texte v2.0.0 → v2.1.0 (Standalone-Wortlaut)
#   5. Verifikation (index.html vollständig, Chunks vorhanden, keine 2.0.0-Reste)
#
...
echo "==> 4/5 Standalone-Version-Patch (v2.0.0 -> v2.1.0)"
find "$WWW" -type f \( -name '*.html' -o -name '*.js' -o -name '*.css' \) -print0 | xargs -0 sed -i \
  -e 's/v2\.0\.0/v2.1.0/g' \
  -e 's/"2\.0\.0"/"2.1.0"/g' \
  -e "s/(nativ, ~100 KB)/(Standalone, ~9 MB)/g" \
  -e 's/Native Android-App + Web Serial + WebUSB/Standalone Android-App (100 % offline) + Web Serial + WebUSB/g'
...
REST="$(find "$WWW" -type f \( -name '*.html' -o -name '*.js' -o -name '*.css' \) -exec grep -l '2\.0\.0' {} + 2>/dev/null || true)"
if [ -n "$REST" ]; then
  echo "WARN: 2.0.0-Reste in:" >&2
  echo "$REST" | head -5 >&2
fi

VOK="$(grep -rl 'v2\.1\.0' "$WWW" --include='*.html' --include='*.js' | wc -l)"
